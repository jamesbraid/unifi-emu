...
// Metadata describes the source firmware and deterministic root filesystem tar.
...
func buildRootFSBundle(decoded decodeResult, source Source, output io.Writer) (Metadata, error) {
	root, ok := selectRootFS(decoded.Roots)
	if !ok {
		return Metadata{}, ErrNoRootFS
	}
	...
	for _, entry := range entries {
		...
		default:
			return Metadata{}, fmt.Errorf("unsupported rootfs entry kind %q for %q", entry.Kind, entry.Path)
		}
		if err := writer.WriteHeader(header); err != nil {
			return Metadata{}, fmt.Errorf("write rootfs tar header %q: %w", name, err)
		}
		if entry.Kind == "regular" {
			if _, err := writer.Write(entry.Data); err != nil {
				return Metadata{}, fmt.Errorf("write rootfs tar file %q: %w", name, err)
			}
		}
	}
	if err := writer.Close(); err != nil {
		return Metadata{}, fmt.Errorf("close rootfs tar: %w", err)
	}
	return Metadata{
		SourceFirmwareDigest: source.Digest,
		Platform:             source.Platform,
		Version:              source.Version,
		NestedArtifactPath:   root.Artifact,
		TarDigest:            fmt.Sprintf("%x", digest.Sum(nil)),
...
func normalizeRootEntries(input []decodedFile) ([]decodedFile, error) {
	...
	slices.SortFunc(entries, func(a, b decodedFile) int { return strings.Compare(a.Path, b.Path) })
	...
	for _, indexes := range groups {
		...
		for _, index := range indexes {
			if entries[index].Kind == "regular" {
				...
				if !bytes.Equal(content, entries[index].Data) {
					return nil, fmt.Errorf(
						"rootfs hardlink group at %q has conflicting payloads",
						entries[indexes[0]].Path,
					)
				}
			}
		}
...
// WriteMetadata writes metadata as stable, indented JSON.
