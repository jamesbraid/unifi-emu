...
func TestDecodeTraversesUBNTExecPayload(t *testing.T) {
	...
	if len(result.Files) != 1 ||
		result.Files[0].Path != "firmware.bin/script/bin/preflash" {
		t.Fatalf("decoded EXEC files = %+v", result.Files)
	}
}
...
func TestDecodeTraversesUBNTFilePayload(t *testing.T) {
	var compressed bytes.Buffer
	gzipWriter := gzip.NewWriter(&compressed)
	if _, err := gzipWriter.Write([]byte("agent")); err != nil {
		t.Fatal(err)
	}
	if err := gzipWriter.Close(); err != nil {
		t.Fatal(err)
	...
	if len(result.Files) != 1 || result.Files[0].Path != "firmware.bin/rootfs" ||
		string(result.Files[0].Data) != "agent" {
		t.Fatalf("decoded FILE files = %+v", result.Files)
	}
}
...
func TestDecodeIgnoresNestedInvalidUBNTHeaderCandidate(t *testing.T) {
	...
	if len(result.Files) != 1 ||
		result.Files[0].Path != "rootfs.tar/lib/ubnt/preloader.bin" {
		t.Fatalf("nested files = %+v", result.Files)
	}
}
