...
func (l Limits) withDefaults() Limits {
	...
	if l.MinStringLength <= 0 {
		l.MinStringLength = defaults.MinStringLength
	}
	return l
}
...
func Decode(data []byte, name string, limits Limits) DecodeResult {
	...
	state.walk(name, data, 0, "", 0)
	return state.result
}

func (s *decoderState) walk(name string, data []byte, offset int64, compression string, depth int) {
	if depth > s.limits.MaxDepth {
		s.fail(name, "decode", fmt.Errorf("nesting exceeds limit %d", s.limits.MaxDepth))
		return
	}
	if len(s.result.Artifacts) >= s.limits.MaxArtifacts {
		s.fail(name, "decode", fmt.Errorf("artifact count exceeds limit %d", s.limits.MaxArtifacts))
		return
	}

	artifactOffset, offsetBasis := offset, "parent"
	if offset < 0 {
		artifactOffset, offsetBasis = 0, ""
	}
	if compression != "" && compression != "none" {
		...
		s.result.Artifacts = append(s.result.Artifacts, Artifact{
			Path: name, Format: compression, Offset: artifactOffset, Size: int64(len(data)),
			OffsetBasis: offsetBasis, Compression: compression,
			SHA256: fmt.Sprintf("%x", sha256.Sum256(data)),
		})
		s.walk(name, decoded, 0, "", depth+1)
		return
	}

	format := detectFormat(data)
	s.result.Artifacts = append(s.result.Artifacts, Artifact{
		Path: name, Format: format, Offset: artifactOffset, OffsetBasis: offsetBasis, Size: int64(len(data)),
		SHA256: fmt.Sprintf("%x", sha256.Sum256(data)),
	})
	switch format {
	case "ubnt":
		...
		for _, child := range children {
			s.walk(joinArtifact(name, child.Name), child.Data, child.Offset, child.Compression, depth+1)
		}
	case "uimage":
	...
	case "fit":
		...
		if len(children) == 0 {
			s.result.Artifacts[len(s.result.Artifacts)-1].Format = "fdt"
			s.result.Files = append(s.result.Files, decodedFile{Path: name, Data: data, Offset: offset})
			return
		}
		for _, child := range children {
			if len(s.result.Artifacts) >= s.limits.MaxArtifacts {
	...
	case "cpio":
		...
		s.result.Roots = append(s.result.Roots, decodedRoot{Artifact: name, Format: format, Entries: files})
		for _, file := range files {
			if file.Kind == "regular" {
				s.addFile(joinArtifact(name, file.Path), file.Data, file.Offset, depth+1)
			}
		}
	case "squashfs":
		archiveLimits, err := s.archiveLimits()
		...
		for _, file := range files {
			if file.Kind == "regular" {
				s.addFile(joinArtifact(name, file.Path), file.Data, 0, depth+1)
			}
		}
	case "tar":
		archiveLimits, err := s.archiveLimits()
	...
	default:
		...
		if hasArchiveBudget {
			if squashOffset, files, ok := findEmbeddedSquashFS(data, archiveLimits); ok {
				if err := s.chargeEntries(files); err != nil {
					s.fail(name, "squashfs", err)
					return
				}
				...
				s.result.Artifacts = append(s.result.Artifacts, Artifact{
					Path: squashPath, Format: "squashfs", Offset: int64(squashOffset),
					OffsetBasis: "parent", Size: int64(len(data) - squashOffset),
					SHA256: fmt.Sprintf("%x", sha256.Sum256(data[squashOffset:])),
				})
				s.result.Roots = append(s.result.Roots, decodedRoot{
					Artifact: squashPath, Format: "squashfs", Entries: files,
				})
				for _, file := range files {
					if file.Kind == "regular" {
						s.addFile(joinArtifact(squashPath, file.Path), file.Data, 0, depth+1)
					}
				}
				return
			}
		...
		if hasArchiveBudget {
			if cpioOffset, files, ok := findEmbeddedCPIO(data, archiveLimits); ok {
				if err := s.chargeEntries(files); err != nil {
					s.fail(name, "cpio", err)
					return
				}
				...
				s.result.Artifacts = append(s.result.Artifacts, Artifact{
					Path: cpioPath, Format: "cpio", Offset: int64(cpioOffset),
					OffsetBasis: "parent", Size: int64(len(data) - cpioOffset),
					SHA256: fmt.Sprintf("%x", sha256.Sum256(data[cpioOffset:])),
				})
				s.result.Roots = append(s.result.Roots, decodedRoot{
					Artifact: cpioPath, Format: "cpio", Entries: files,
				})
				for _, file := range files {
					if file.Kind == "regular" {
						s.addFile(joinArtifact(cpioPath, file.Path), file.Data,
							file.Offset, depth+1)
					}
				}
				return
		...
		if embeddedOffset := findEmbeddedXZ(data); embeddedOffset >= 0 {
			s.walk(joinArtifact(name, "embedded.xz"), data[embeddedOffset:],
				int64(embeddedOffset), "", depth+1)
			return
		}
		s.result.Files = append(s.result.Files, decodedFile{Path: name, Data: data, Offset: offset})
	}
...
func (s *decoderState) addFile(name string, data []byte, offset int64, depth int) {
	if len(s.result.Artifacts) >= s.limits.MaxArtifacts {
		s.fail(name, "decode", fmt.Errorf("artifact count exceeds limit %d", s.limits.MaxArtifacts))
		return
	}
	...
	s.result.Artifacts = append(s.result.Artifacts, Artifact{
		Path: name, Format: format, Offset: offset, OffsetBasis: "parent", Size: int64(len(data)),
		SHA256: fmt.Sprintf("%x", sha256.Sum256(data)),
	})
	s.result.Files = append(s.result.Files, decodedFile{Path: name, Data: data, Offset: offset})
}
...
func (s *decoderState) archiveLimits() (Limits, error) {
	limits, ok := s.archiveLimitsIfAvailable()
	if !ok {
...
func (s *decoderState) archiveLimitsIfAvailable() (Limits, bool) {
	...
	limits := s.limits
	limits.MaxExpandedBytes = remaining
	return limits, true
}
...
func findEmbeddedCPIO(data []byte, limits Limits) (int, []decodedFile, bool) {
	...
	for searchFrom := 1; searchFrom < len(data); {
		...
		offset := searchFrom + relative
		if files, err := parseCPIO(data[offset:], limits); err == nil && len(files) > 0 {
			return offset, files, true
		}
		searchFrom = offset + 1
	}
