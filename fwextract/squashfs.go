...
func (w *squashFSWalker) walkDirectory(directory fs.ReadDirFile, parent string) (err error) {
	...
	for {
		remaining := w.limits.MaxArtifacts - len(w.files)
		readSize := squashFSReadDirChunk
		...
		if err := w.walkEntries(entries, parent); err != nil {
			return err
		}
		if readErr == io.EOF {
			return nil
		...
		if len(entries) == 0 {
			// KarpelesLab/squashfs returns an empty slice and nil at end of
			// directory even when ReadDir is called with a positive count.
			return nil
		}
	}
}

func (w *squashFSWalker) walkEntries(entries []fs.DirEntry, parent string) error {
	for _, entry := range entries {
		...
		if info.Size() > 0 {
			...
			if readErr != nil {
				readErr = fmt.Errorf("read SquashFS entry %q: %w", name, readErr)
			}
		}
